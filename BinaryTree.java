//BinaryTree.java
//Jenny Chen
//make the methods in the Binary Tree Assignment
...
class BTree{
	...
	private BNode find(int val,BNode branch){
	...
	public String displayPre(BNode branch){
		if(branch == null){
			...
			return branch.getVal()+ " " + displayPre(branch.getLeft()) + displayPre(branch.getRight());
		}
	}
	...
	public String displayPost(BNode branch){
		if(branch == null){
			...
			return displayPost(branch.getLeft()) +  displayPost(branch.getRight()) + branch.getVal()+ " ";
		}
	...
	public boolean isAncestor(int a, int d){
	...
	public void graft(BNode branch, BNode trunk){
		...
		else if(branch.getVal()>trunk.getVal()){
	...
	public BNode findBefore(int v){
		return findBefore(v,root);
	}
	
	//recursive overload of findBefore
	public BNode findBefore(int v, BNode branch){
		//base case: find the BNode that points to a BNode with the value v
		if(branch.getLeft() != null && branch.getLeft().getVal() == v){	
			return branch;
		}
		if(branch.getRight() != null&& branch.getRight().getVal() == v){
			return branch;
		}
